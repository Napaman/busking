...
describe(CollectionConfig.contractName, function () {
  ...
  it('Check initial data', async function () {
    ...
    expect(await contract.buskingMetadataUri()).to.equal(CollectionConfig.buskingMetadataUri);
  ...
  it('Owner only functions', async function () {
    ...
    await expect(contract.connect(externalUser).setbuskingMetadataUri('INVALID_URI')).to.be.revertedWith('Ownable: caller is not the owner');
  ...
  it('Token URI generation', async function () {
    ...
    expect(await contract.tokenURI(1)).to.equal(CollectionConfig.buskingMetadataUri);
